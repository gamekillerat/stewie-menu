...
namespace Menu {
	...
	namespace Settings {
		...
		extern int CurrentBackgroundScroll;
	...
	void Speedometer(char* text, short line);
	void fps(char* text);
	...
	bool Break(const char* title);
	bool Option(const char* option);
	void PlayerInfoTitle(char* text);
	void MenuNameAndVersion(char* text);
